...
在 RL、机器人、仿真、AutoML 和元学习项目里，一个结果通常受多个因素共同影响：

```text
奖励函数、观测空间、动作空间、训练算法、超参数、场景分布、随机种子、评估协议
```

不要一上来把多个因素一起改完再解释结果。推荐顺序是：

1. 先列出可能影响结果的因素清单；
2. 按优先级或耦合强度选择单一因素；
3. 保持其他因素稳定，验证该因素的主效应；
4. 单因素结果稳定后，再进入组合实验；
5. 组合实验必须能追溯到对应的单因素基线。

如果多个因素同时变化导致无法归因，把失败分类为 `factor_confounding`，退回最近的单因素基线或拆成更小的决策节点。
...
- Controlled factors:
- Changed factors:
- Single-factor baselines for combination runs:
- Algorithm:
- Hyperparameters:
- Seeds:
- Training steps:
...
奖励、观测和学习率同时改变后指标提升：可能是 factor_confounding，不能判断提升来自哪个因素。
...
6. 每个组合实验都能追到对应的单因素基线；
7. 每个“提升”都要问：是机制提升，还是实现细节、数据、评估、因素混杂或随机性造成的。
